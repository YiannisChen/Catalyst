...
The eval frozen corpus (`data/catalyst_eval_frozen.db`, table `clean_assets`) is the authoritative set of textual rows to embed for defense-grade reproducibility.
...
- **Artifact layout (normative for P1-T01 acceptance):**
  - `data/embeddings/bge_m3_eval_frozen.npy` — row-aligned matrix or agreed serialized layout documented in the build script README.
...
- **Coverage invariant:** Embedding index must cover **all** `clean_assets` rows in `catalyst_eval_frozen.db` that participate in Gold index rows for eval (plan: **1:1 embedding row per qualifying `clean_assets` row**; gaps are failures, not silent skips).
