...
- Current frozen comparison shows both configs at `evidence_validity=1.0`, `schema_validity=1.0`, `trace_completeness=1.0`, and `should_refuse_hit_rate=1.0`.
- `mcj_full` is more expensive and slower than `direct_llm`, which is expected because it executes Critic, Judge, Validator, and trace instrumentation.
...
- Artifacts are deterministic by convention: `data/eval_reports/<frozen_ts>_comparison.{json,md}` and `data/traces/<run_id>.json`.
