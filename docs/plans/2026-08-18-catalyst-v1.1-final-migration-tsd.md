...
**Repository:** `/Users/yiannischen/Desktop/Catalyst` (current authority; historical `/Users/yiannischen/Projects/Catalyst` paths are provenance only)
...
### Milestone 8 — Causal-quality recovery, cleanup, and release candidate

**Dated ordering amendment (2026-09-18):** the sealed M7 development run is a
valid negative result: operational execution completed, but the served
generation supplied no citable full-text evidence and all 12 cases abstained.
M8 therefore executes M8-A–D recovery before M8-E cleanup. This reuses the M3
architecture and does not change the V1.1 production graph. Cleanup is M8-E;
M8-B is the CPU retrieval quality gate.

| Field | Contract |
|---|---|
| WHAT BECOMES TRUE | One understandable default path remains and Stage-2 readiness is evidenced |
| DEPENDENCIES | immutable M7 failed report; M3 rebuild/promotion path; every named deletion gate |
| MAJOR WORK | M8-A: general body-bearing generation. M8-B: CPU retrieval gate. M8-C: FTS attribution probe. M8-D: dense/hybrid promotion and new Stage-1 recovery report. M8-E: remove shims/legacy executable paths/stale pointers/temp tools; finalize CI tiers, rollback and retention docs |
| EXIT GATE | Passing sealed M8-D retrieval/attribution gates under new identities; immutable M7 report reproduction; repository-wide FAST/MEDIUM, selected EXPENSIVE gates, clean imports, diff audit |
...
→ failed M7 12-case Stage-1 report retained as negative evidence
→ M8-A body-bearing generation
→ M8-B CPU retrieval quality
→ M8-C FTS attribution probe
→ M8-D dense/hybrid Stage-1 recovery
→ M8-E cleanup/release candidate
```

| Class | Work |
|---|---|
| CRITICAL | M1 baseline; canonical/SEC/news/dedup repair; rebuilt retrieval; foundation artifacts; Analyst/claims/Writer; event/artifact transactions; SSE/replay; primary Workbench; GoldenCase/EvalManifest Stage 1; M8 generation-content recovery before RC |
