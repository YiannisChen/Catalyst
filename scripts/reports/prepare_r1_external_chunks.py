...
def _resolve_chunk_text(
    *,
    ...
    """Return (chunk_text, fallback_to_full_content, missing_text)."""
    m = _L2_PAT.match(chunk_id)
    if not m:
        content = _load_asset_content(conn, chunk_id)
    ...
    fallback_text = str(parent_content or "").strip()
    ...
    try:
        for sp in summary_paths:
            ...
            for score in (summary.get("critic", {}).get("all_graded_scores", []) or []):
                ...
                if not chunk_id:
                    continue
                chunk_text, fallback, missing_text = _resolve_chunk_text(conn=conn, splitter=splitter, chunk_id=chunk_id)
                chunks.append(
                    {
                        "chunk_id": chunk_id,
...
def main() -> int:
    ...
    parser.add_argument(
        "--strict-empty-text",
        action="store_true",
        help="Exit non-zero when any chunk has missing/empty chunk_text.",
    )
    ...
    empty_count = sum(
        ...
        if (not str(chunk.get("chunk_text", "")).strip()) or bool(chunk.get("missing_text"))
    )

    print(f"[ok] output={out_path}")
    print(f"[ok] rows={len(rows)}")
    print(f"[info] empty_or_missing_chunk_text={empty_count}")
    if empty_count > 0:
        print("[warn] detected empty/missing chunk_text rows; review fallback and source coverage.")
        if args.strict_empty_text:
            print("[fail] strict mode enabled and empty/missing chunk_text > 0")
            return 2
    return 0
