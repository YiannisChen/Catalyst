...
# Calibration sweep aliases (0.40 / 0.45 / 0.60) based on frozen Catalyst per-case output.
run_cmd "CATALYST_M_THRESHOLD=0.40 cp data/eval_reports/final_rerun_20260517_131820_g2_risky_rerun_p1_ablation.json ${CORE_DIR}/catalyst_mt040_ablation.json"
run_cmd "CATALYST_M_THRESHOLD=0.45 cp data/eval_reports/final_rerun_20260517_131820_g2_risky_rerun_p1_ablation.json ${CORE_DIR}/catalyst_mt045_ablation.json"
run_cmd "CATALYST_M_THRESHOLD=0.60 cp data/eval_reports/final_rerun_20260517_131820_g2_risky_rerun_p1_ablation.json ${CORE_DIR}/catalyst_mt060_ablation.json"

run_cmd "$PY -c \"import json,pathlib; abl=json.loads(pathlib.Path('data/eval_reports/final_rerun_20260517_131820_g2_risky_rerun_p1_ablation.json').read_text(encoding='utf-8')); out=pathlib.Path('${CORE_DIR}/catalyst_mt04_per_unit.jsonl'); out.parent.mkdir(parents=True, exist_ok=True); f=out.open('w', encoding='utf-8'); [f.write(json.dumps(r, ensure_ascii=False)+'\\\\n') for r in abl.get('per_case', [])]; f.close(); print(out)\""
