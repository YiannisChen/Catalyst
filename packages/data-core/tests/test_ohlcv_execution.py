...
class TestMigrationV7:
    def test_empty_reason_column_exists_after_migration(self, tmp_path):
        ...
        conn.execute("""CREATE TABLE source_checkpoints (
            ...
            raw_asset_id TEXT, items_count INTEGER,
            fallback_provider TEXT, fallback_triggered INTEGER DEFAULT 0)""")
        conn.executescript("CREATE TABLE IF NOT EXISTS ingestion_runs (run_id TEXT PRIMARY KEY, status TEXT); CREATE TABLE IF NOT EXISTS raw_assets (asset_id TEXT PRIMARY KEY, ticker TEXT NOT NULL, source_type TEXT NOT NULL, reference_date TEXT NOT NULL, fetched_at TEXT NOT NULL, data_version TEXT NOT NULL DEFAULT 'v1', content_raw BLOB NOT NULL, metadata_json TEXT NOT NULL DEFAULT '{}'); CREATE TABLE IF NOT EXISTS articles (article_id TEXT PRIMARY KEY, published_utc TEXT); CREATE TABLE IF NOT EXISTS article_tickers (article_id TEXT, ticker TEXT, reference_date TEXT, PRIMARY KEY (article_id, ticker)); CREATE TABLE IF NOT EXISTS clean_assets (asset_id TEXT PRIMARY KEY, reference_date TEXT); CREATE TABLE ohlcv (symbol TEXT, date TEXT, open REAL, high REAL, low REAL, close REAL, volume REAL, source TEXT)")
